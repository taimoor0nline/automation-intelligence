 ...
 *
 * Two code paths:
 *  - buildTestBody(): the 24 hand-authored templates (TC001-TC024), specific
 *    to the demo Customer Feedback form.
 *  - buildCustomTestBody(): a GENERIC generator for user-added custom test
 *    cases (from the "+ Add Test Case" UI). It only knows a field's testId,
 *    what action to perform, and what to assert — so it works for any field
 *    Page Discovery found, not just the demo form's fixed fields.
 */
...
// Builds the body of one `it(...)` block for a hand-authored (TC001-TC024) test case.
...
// Finds a reasonable submit-button selector from whatever Page Discovery found,
// so custom test cases work on pages other than the demo form too.
function resolveSubmitSelector(pageDiscovery) {
  const btn = (pageDiscovery?.elements || []).find((el) => el.tag === "button");
  if (btn?.testId) return `[data-testid="${btn.testId}"]`;
  if (btn?.text) return `button:contains("${btn.text.replace(/"/g, '\\"')}")`;
  return 'button[type="submit"]';
}

// GENERIC builder for user-added custom test cases. Works off only:
//   action:    { fieldTestId, fieldName, type: "type"|"select"|"check"|"uncheck", value }
//   assertion: { kind: "errorVisible"|"noError"|"successVisible", targetTestId, message }
// This is what lets "+ Add Test Case" work on ANY discovered field, not just
// the 24 pre-authored feedback-form cases above.
function buildCustomTestBody(tc, submitSelector, pagePath) {
  const nav = `  cy.visit('${pagePath || "/"}');`;
  const lines = [nav];
  ...
  if (!fieldSelector) {
    lines.push(`  // No usable selector for this field — skipped. Add a data-testid or name attribute to the target field.`);
    return lines.join("\n");
  }

  if (act.type === "select") {
    lines.push(`  cy.get('${fieldSelector}').select(${JSON.stringify(act.value ?? "")});`);
  } else if (act.type === "check") {
    lines.push(`  cy.get('${fieldSelector}').check();`);
  } else if (act.type === "uncheck") {
    lines.push(`  cy.get('${fieldSelector}').uncheck();`);
  } else {
    lines.push(`  cy.get('${fieldSelector}').clear().type(${JSON.stringify(act.value ?? "")});`);
  }

  lines.push(`  cy.get('${submitSelector}').click();`);

  const a = tc.assertion || {};
  const targetSelector = a.targetTestId ? `[data-testid="${a.targetTestId}"]` : null;

  if (a.kind === "errorVisible") {
    if (targetSelector && a.message) {
      lines.push(`  cy.get('${targetSelector}').should('contain.text', ${JSON.stringify(a.message)});`);
    } else if (targetSelector) {
      lines.push(`  cy.get('${targetSelector}').should('be.visible');`);
    } else if (a.message) {
      lines.push(`  cy.contains(${JSON.stringify(a.message)}).should('be.visible');`);
    } else {
      lines.push(`  // No assertion target/message provided for this error check.`);
    }
  } else if (a.kind === "noError") {
    if (targetSelector) {
      lines.push(`  cy.get('${targetSelector}').should('have.text', '');`);
    } else {
      lines.push(`  // No target element provided to confirm absence of an error.`);
    }
  } else if (a.kind === "successVisible") {
    if (a.message) {
      lines.push(`  cy.contains(${JSON.stringify(a.message)}).should('be.visible');`);
    } else if (targetSelector) {
      lines.push(`  cy.get('${targetSelector}').should('be.visible');`);
    } else {
      lines.push(`  // No success indicator provided to check.`);
    }
  }
...
function buildTestBody(tc, submitSelector, pagePath) {
  if (tc.custom) return buildCustomTestBody(tc, submitSelector, pagePath);
  return buildTemplateTestBody(tc);
}

function mockGenerateCypressCode({ approvedTestCases, pageDiscovery, fileName }) {
  const submitSelector = resolveSubmitSelector(pageDiscovery);
  const pagePath = (() => {
    try { return new URL(pageDiscovery?.url).pathname || "/"; } catch { return "/"; }
  })();
...
describe('Customer Feedback', () => {`;
  ...
  const body = approvedTestCases
    .map((tc) => `  it('${tc.id}: ${tc.title.replace(/'/g, "\\'")}', () => {\n${buildTestBody(tc, submitSelector, pagePath)}\n  });`)
    .join("\n\n");
