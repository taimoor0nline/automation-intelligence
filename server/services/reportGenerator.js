...
function tcIdFromTitle(title) {
  return String(title || "").match(/TC(?:\d{3}|-C\d+)/)?.[0] || "—";
}
...
<style>
  ...
  *{box-sizing:border-box} body{margin:0;background:var(--bg);color:var(--text);font-family:Inter,Segoe UI,Arial,sans-serif}
  .wrap{max-width:1180px;margin:0 auto;padding:30px 22px 50px}.hero{display:flex;justify-content:space-between;gap:24px;align-items:flex-start;margin-bottom:24px}
...
<body><div class="wrap">
  ...
  <table><thead><tr><th>Case</th><th>Test</th><th>Status</th><th>Duration</th><th>AI analysis / error</th></tr></thead><tbody>${rows}</tbody></table>
</div></body></html>`;
}
