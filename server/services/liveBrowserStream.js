...
function attachToTarget(target) { return new Promise((resolve, reject) => { const socket = new WebSocket(target.webSocketDebuggerUrl); ws = socket; let commandId = 1; let lastFrameSentAt = 0; socket.once("open", () => { setStatus("live", `Streaming ${target.title || "automation browser"}`); socket.send(JSON.stringify({ id: commandId++, method: "Page.enable" })); socket.send(JSON.stringify({ id: commandId++, method: "Page.startScreencast", params: { format: "jpeg", quality: 65, maxWidth: 1280, maxHeight: 720, everyNthFrame: 1 } })); resolve(); }); socket.on("message", (raw) => { let message; try { message = JSON.parse(String(raw)); } catch { return; } if (message.method !== "Page.screencastFrame") return; const sessionId = message.params?.sessionId; if (sessionId != null && socket.readyState === WebSocket.OPEN) socket.send(JSON.stringify({ id: commandId++, method: "Page.screencastFrameAck", params: { sessionId } })); const now = Date.now(); if (now - lastFrameSentAt < 180) return; lastFrameSentAt = now; if (message.params?.data) pushFrame(message.params.data); }); socket.once("error", reject); socket.once("close", () => { if (!stopRequested) setStatus("waiting", "Automation browser disconnected. Waiting for the next browser target."); if (ws === socket) ws = null; }); }); }
...
function start(infoFile) { stopRequested = false; latestFrame = null; latestFrameAt = null; setStatus("starting", "Starting live browser stream..."); if (!connectLoopPromise) connectLoopPromise = connectionLoop(infoFile).finally(() => { connectLoopPromise = null; }); }
