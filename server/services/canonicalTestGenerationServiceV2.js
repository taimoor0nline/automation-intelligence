...
- For a submit expected to succeed, populate all discovered required or validation-bearing controls in that form. Do not omit a checkbox/radio group merely because HTML required is absent when discovery links it to a validation error element.
...
async function generateCanonicalBatch({
  ...
  for (const unit of plannedUnits) {
    ...
    const behavior = normalizeBehavioralIr(rawCanonicalIr, { registry, plannedUnit: unit, story });
    if (behavior.unresolved.length) {
      const error = new Error(`${unit.plannedId} contains an ungrounded runtime-behavior assumption: ${behavior.unresolved[0]}`);
      error.code = 'CANONICAL_BEHAVIOR_UNGROUNDED';
      error.plannedId = unit.plannedId;
      error.validationErrors = behavior.unresolved;
      throw error;
    }
    const canonicalIr = behavior.ir;
    ...
    testCases.push({
      ...
      canonicalValidation: {
        ...
        behavioralGrounding: canonicalIr.behavioralGrounding,
      },
      behavioralGrounding: canonicalIr.behavioralGrounding,
      workflowRequirements: actorRuntime.workflowRequirements || null,
      cypressPreview,
