...
const CAPABILITY_RULES = [
  {
    pattern: /\b(captcha|recaptcha|hcaptcha)\b/i,
    status: NOT_AUTOMATABLE,
    reasonCode: "CAPTCHA_REQUIRED",
    resolutionType: RESOLUTION_MANUAL_TESTING,
    reason: "The scenario requires a CAPTCHA challenge, which the configured automation system intentionally does not automate.",
  },
  {
    pattern: /\b(face\s?id|fingerprint|biometric|touch\s?id)\b/i,
    status: NOT_AUTOMATABLE,
    reasonCode: "BIOMETRIC_REQUIRED",
    resolutionType: RESOLUTION_MANUAL_TESTING,
    reason: "The scenario requires native biometric interaction, which is outside the configured web automation system.",
  },
  {
    pattern: /\b(native\s+file\s+(?:dialog|picker)|windows\s+(?:dialog|prompt)|os\s+(?:dialog|prompt))\b/i,
    ...
    reason: "The scenario requires a native operating-system dialog that the configured automation system does not control.",
  },
  {
    pattern: /\b(browser\s+extension|extension\s+popup)\b/i,
    status: REQUIRES_FRAMEWORK_CAPABILITY,
    reasonCode: "BROWSER_EXTENSION_REQUIRED",
    resolutionType: RESOLUTION_FRAMEWORK_CHANGE_REQUIRED,
    reason: "The scenario requires browser-extension UI outside the configured application automation surface.",
  },
  {
    pattern: /\b(native\s+mobile|android\s+app|ios\s+app|mobile\s+app)\b/i,
    status: REQUIRES_FRAMEWORK_CAPABILITY,
    reasonCode: "NATIVE_MOBILE_REQUIRED",
    resolutionType: RESOLUTION_FRAMEWORK_CHANGE_REQUIRED,
    reason: "The scenario targets a native mobile application and requires a mobile automation capability.",
  },
  {
    pattern: /\b(camera\s+permission|microphone\s+permission|system\s+permission\s+prompt)\b/i,
    status: REQUIRES_FRAMEWORK_CAPABILITY,
    reasonCode: "SYSTEM_PERMISSION_PROMPT_REQUIRED",
    resolutionType: RESOLUTION_FRAMEWORK_CHANGE_REQUIRED,
    reason: "The scenario depends on a browser or operating-system permission prompt outside the current deterministic interaction contract.",
  },
];

function result({ status, automatable, reasonCode, reason, reasons = [], resolutionType, requiredInputs = [], evidence = [] }) {
  return {
    status,
    automatable,
    reasonCode,
...
function classifyTestCase(testCase, { pageDiscoveries = [], hasCredentials = false } = {}) {
  if (!testCase || typeof testCase !== "object") {
    return result({
      ...
      automatable: false,
      reasonCode: "MALFORMED_TEST_CASE",
      reason: "The test case is missing or malformed.",
      resolutionType: RESOLUTION_AI_REPAIRABLE,
    });
  }

  if (!String(testCase.title || "").trim()) {
    return result({ status: INVALID_TEST_CASE, automatable: false, reasonCode: "MISSING_TITLE", reason: "A test-case title is required.", resolutionType: RESOLUTION_AI_REPAIRABLE });
  }
  if (!Array.isArray(testCase.steps) || !testCase.steps.length) {
    return result({ status: INVALID_TEST_CASE, automatable: false, reasonCode: "MISSING_STEPS", reason: "At least one executable test step is required.", resolutionType: RESOLUTION_AI_REPAIRABLE });
  }
  if (!Array.isArray(testCase.expectedResults) || !testCase.expectedResults.length) {
    return result({ status: INVALID_TEST_CASE, automatable: false, reasonCode: "MISSING_EXPECTED_RESULTS", reason: "At least one expected result is required.", resolutionType: RESOLUTION_AI_REPAIRABLE });
  }

  const fullText = JSON.stringify(testCase);
  for (const rule of CAPABILITY_RULES) {
    if (rule.pattern.test(fullText)) {
      return result({ ...rule, automatable: false });
    }
  }
  ...
  if (requiresRuntimeCredentials(testCase) && !hasCredentials) {
    return result({
      ...
      reason: "Valid runtime credentials are required for this test case but have not been supplied.",
      resolutionType: RESOLUTION_USER_INPUT_REQUIRED,
  ...
  if (selectorProblems.length) {
    return result({
      status: INSUFFICIENT_EVIDENCE,
      automatable: false,
      reasonCode: "UNDISCOVERED_SELECTOR",
      reason: `The test references a selector that was not verified during page discovery: ${selectorProblems[0]}`,
      reasons: selectorProblems.map((selector) => `Undiscovered selector: ${selector}`),
      resolutionType: RESOLUTION_AI_REPAIRABLE,
      evidence: [...discovery.selectors].slice(0, 50),
    });
  }
  ...
  if (pathProblems.length) {
    return result({
      status: INSUFFICIENT_EVIDENCE,
      automatable: false,
      reasonCode: "UNDISCOVERED_PATH",
      reason: `The test references a navigation path that was not verified during page discovery: ${pathProblems[0]}`,
      reasons: pathProblems.map((path) => `Undiscovered navigation path: ${path}`),
      resolutionType: RESOLUTION_AI_REPAIRABLE,
      evidence: [...discovery.paths].slice(0, 30),
    });
  }
  ...
  return result({
    status: READY,
    automatable: true,
    reasonCode: "SUPPORTED_AND_GROUNDED",
    reason: "All required interactions are supported by the automation system and grounded in discovered application evidence.",
