...
const { proposeGenerationPlan } = require('../services/progressiveTestGenerator');
const { generateCanonicalBatch } = require('../services/canonicalTestGenerationService');
...
function buildGenerationBatches(units) {
  ...
  for (const grouped of groups.values()) {
    for (let offset = 0; offset < grouped.length; offset += GENERATION_BATCH_SIZE) batches.push({ units: grouped.slice(offset, offset + GENERATION_BATCH_SIZE) });
  }
...
async function persistCanonicalStage(job, session, stage, cases = []) {
  ...
  try {
    ...
    if (stage === 'plan') await canonicalArtifacts.persistPlan(job.sessionId, session.canonicalGenerationPlan);
    if (stage === 'cases') {
      await canonicalArtifacts.persistCases(job.sessionId, cases);
      await persistence.persistTestCases(job.sessionId, cases);
    }
    if (stage === 'all') {
      await canonicalArtifacts.persistAll(job.sessionId, session);
  ...
  function pump() {
    while (active < READINESS_CONCURRENCY && queue.length) {
      ...
      emit(job, 'READINESS_STARTED', {
        testCaseId: tc.id,
        plannedId: tc.canonicalIr?.plannedId || null,
        checking: active,
        completed,
      ...
      setImmediate(() => {
        try {
          ...
          completed += 1;
          updateSession();
          emit(job, 'READINESS_FAILED', { testCaseId: tc.id, plannedId: tc.canonicalIr?.plannedId || null, message: err.message, completed, generated: slots.filter(Boolean).length });
        } finally {
          active -= 1;
          pump();
          notifyDrain();
...
async function runGeneration(job, input) {
  ...
  try {
    ...
    emit(job, 'CANONICAL_REGISTRY_READY', {
      registryHash: registry.registryHash,
      pageCount: registry.pages.length,
      elementCount: registry.elements.length,
      databaseMode: canonicalArtifacts.enabled() ? 'POSTGRESQL_AND_SESSION' : 'SESSION_ONLY',
    });
    await persistCanonicalStage(job, session, 'registry');
    ...
    const units = coveragePlan.units.map((unit, index) => ({
      index,
      plannedId: `P${String(index + 1).padStart(3, '0')}`,
      category: unit.category,
      scenarioType: unit.scenarioType,
      objective: unit.rationale,
    ...
    session.canonicalGenerationPlan = {
      version: 1,
      architecture: 'CANONICAL_IR_V1',
      registryHash: registry.registryHash,
      story: input.story,
    ...
    await persistCanonicalStage(job, session, 'plan');
    ...
    function existingTitles() { return slots.filter(Boolean).map((tc) => tc.title); }
    function recordUnitFailure(unit, err, batchNumber) {
      ...
      emit(job, 'BATCH_FAILED', { ...failure, generatedSoFar: completedCount, totalRequested: plannedCount });
    }
    ...
    async function generateCompatibleBatch(batch, workerNumber, inheritedDepth = 0) {
      ...
      for (let attempt = 1; attempt <= GENERATION_BATCH_MAX_ATTEMPTS; attempt += 1) {
        ...
        emit(job, 'BATCH_STARTED', {
          ...
          plannedIds: batch.units.map((unit) => unit.plannedId),
        ...
        try {
          const generated = await withBatchTimeout(generateCanonicalBatch({
            story: input.story,
            registry,
            ...
            modelTier: input.aiModelTier,
            hasCredentials: Boolean(session.credentials?.username && session.credentials?.password),
            securitySubcategories: first.category === 'SECURITY' ? input.securitySubcategories : [],
            securitySeverities: first.category === 'SECURITY' ? input.securitySeverities : [],
          }), batch);
          feature ||= generated.feature;
          const rawCases = Array.isArray(generated.testCases) ? generated.testCases : [];
          if (rawCases.length !== batch.units.length) throw new Error(`Canonical generator returned ${rawCases.length}/${batch.units.length} cases for batch ${batch.batchNumber}.`);

          const normalizedTitles = rawCases.map((raw) => String(raw?.title || '').trim().toLowerCase());
          if (new Set(normalizedTitles).size !== normalizedTitles.length) throw new Error(`AI returned duplicate titles inside canonical batch ${batch.batchNumber}.`);
          ...
          for (const tc of completedCases) {
            emit(job, 'CANONICAL_IR_VALIDATED', {
              testCaseId: tc.id,
              plannedId: tc.canonicalIr?.plannedId || null,
              irVersion: tc.canonicalIr?.version || 1,
              registryHash: tc.canonicalValidation?.registryHash || registry.registryHash,
              actionCount: tc.canonicalIr?.actions?.length || 0,
              assertionCount: tc.canonicalIr?.assertions?.length || 0,
            });
          }
          ...
          void persistCanonicalStage(job, session, 'cases', completedCases);
          return true;
        } catch (err) {
          lastError = err;
    ...
    if (!generatedCases.length) {
      ...
      const error = new Error(firstFailure?.message || 'AI could not generate any of the planned canonical test cases.');
      error.code = firstFailure?.code || 'AI_GENERATION_ALL_BATCHES_FAILED';
      throw error;
    }

    if (generationFailures.length) {
      const failureGaps = generationFailures.map((item) => `Planned case ${item.plannedId} (${item.category}/${item.scenarioType}) was not generated: ${item.message}`);
      session.coverageProposal = {
        ...session.coverageProposal,
    ...
    console.log(`[canonical-generation] session=${job.sessionId} planned=${plannedCount}/${MAX_CASE_LIMIT} batches=${batches.length} batchSize=${GENERATION_BATCH_SIZE} generated=${cases.length} generationFailures=${generationFailures.length} ready=${session.automationReadiness.ready}/${cases.length} db=${canonicalArtifacts.enabled() ? 'on' : 'off'} total=${Date.now() - startedAt}ms`);
  } catch (err) {
    ...
    console.error(`[canonical-generation] session=${job.sessionId} failed:`, err);
  }
}
...
router.post('/api/generation/start', allowQaManager, (req, res) => {
  ...
  try {
    ...
    session.canonicalElementRegistry = null;
    session.canonicalGenerationPlan = null;
    session.canonicalArchitectureVersion = 1;
