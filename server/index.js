...
const testCaseRepairWorkbenchRoutes = require("./routes/testCaseRepairWorkbench");
...
const behaviorRuleRoutes = require("./routes/behaviorRules");
...
function behavioralGroundingFor(testCase = {}) {
  return testCase.behavioralGrounding || testCase.canonicalValidation?.behavioralGrounding || testCase.automationReadiness?.automationPlan?.behavioralGrounding || null;
}
...
qwen.analyzeFailure = async (args) => {
  ...
  if (ctx.repositoryId) {
    try {
      sourceContext = await buildSourceContext({ repositoryId: ctx.repositoryId, testCase: args?.testCase, expected: args?.expected, actual: args?.actual, analysis: null });
    } catch (err) {
  ...
  if (String(tc.source || '').toLowerCase() === 'ai-canonical' && grounding?.status !== 'GROUNDED' && String(analysis?.classification || '').toUpperCase() === 'APPLICATION_DEFECT') {
    return {
      ...analysis,
      ...
      resolutionComment: 'Regenerate or revalidate this canonical test with the current behavioral-grounding pipeline, then re-run the original business scenario.',
...
app.use((req, res, next) => {
  ...
  const qaOnly = req.path === '/api/chat' || req.path.startsWith('/api/test-cases') || req.path.startsWith('/api/test-rules') || req.path.startsWith('/api/live-browser') || req.path.startsWith('/api/rest') || req.path === '/api/reset';
...
app.use((req, res, next) => { const originalJson = res.json.bind(res); res.json = (payload) => originalJson(sanitizePublicPayload(payload)); next(); });
...
app.use(testCaseRepairWorkbenchRoutes);
app.use(behaviorRuleRoutes);
...
app.get("/health", async (_req, res) => {
  ...
  res.json({ ok: !db.isRequired() || database.connected === true, aiConnected: qwen.isConfigured(), database, authRequired: AUTH_REQUIRED, sourceAwareEnabled: db.isConfigured(), defaultAiProfile: process.env.AI_MODEL_DEFAULT || "strong" });
});
app.get("/live", (_req, res) => res.sendFile(path.join(__dirname, "..", "testpilot-ui", "live.html")));
...
.editor-readiness strong{font-size:11.5px}.editor-readiness .history{margin-top:7px;padding-top:7px;border-top:1px solid var(--border);color:#64748b}
