(function(){
  ...
  const configs=[
    ...
    {root:'#generationCategoryMenu',item:'input[data-test-category]',all:'#generationCategorySelectAll',button:'#generationCategoryButton',count:'#generationCategoryCount',storage:'aiTestPilotGenerationCategories',allText:'All test categories',noun:'category'},
  ...
  function sync(c){
    ...
    const items=[...root.querySelectorAll(c.item)],selected=items.filter(x=>x.checked).map(x=>x.value),isAll=items.length>0&&selected.length===items.length;
  ...
  function install(){
    for(const c of configs){
      ...
      all.addEventListener('change',e=>{e.stopImmediatePropagation();root.querySelectorAll(c.item).forEach(x=>x.checked=all.checked);sync(c);},true);
  ...
  document.addEventListener('click',e=>{
    ...
    const categories=[...document.querySelectorAll('#generationCategoryMenu input[data-test-category]:checked')];
    const types=[...document.querySelectorAll('#generationTypeMenu input[data-scenario-type]:checked')];
    if(categories.length&&types.length)return;
    e.preventDefault();e.stopImmediatePropagation();
    const message=!categories.length?'Select at least one Test Category before generating test cases.':'Select at least one Scenario Type before generating test cases.';
